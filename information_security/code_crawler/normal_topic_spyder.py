...
def insert_data(elems, book_name_xls, name, yuedu, taolun):
    for elem in elems:
        ...
        value1 = [[rid, weibo_username, weibo_userlevel, weibo_content, shares, comments, likes, weibo_time, name, name, yuedu, taolun]]
        print(f"当前插入第{rid}条数据")
        save.write_excel_xls_append_norepeat(book_name_xls, value1)

def get_current_weibo_data(driver, book_name_xls, name, yuedu, taolun, maxWeibo):
    after = 0
    n = 0
    total_scrolls = 0
    while True:
...
def spider(username, password, driver, book_name_xls, sheet_name_xls, keyword, maxWeibo):
    ...
    time.sleep(2)
    get_current_weibo_data(driver, book_name_xls, keyword, yuedu, taolun, maxWeibo)
    time.sleep(2)
